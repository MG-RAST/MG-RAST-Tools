...
import json
from mglib import async_rest_api, get_auth_token, obj_from_url
...
if __name__ == '__main__':
    ...
    parser.add_argument("-1", "--one", dest="onlyonequery", action="store_true")
...
# retrieve the data by sending at HTTP GET request to the MG-RAST API
    if not opts.onlyonequery:
        jsonstructure = async_rest_api(URI, auth=key)
    else:
        jsonstructure = obj_from_url(URI, auth=key)
